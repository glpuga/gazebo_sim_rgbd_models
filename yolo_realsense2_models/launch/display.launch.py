...
def generate_launch_description():
    urdf_models_path = os.path.join(
        get_package_share_directory("yolo_realsense2_models"), "urdf", "testbenchs"
    )
    available_models = [
    ...
    camera_model_conf = LaunchConfiguration("camera_model")

    rviz_config_filepath = PathJoinSubstitution(
        [FindPackageShare("yolo_realsense2_models"), "rviz", "display.rviz"]
    )
    ...
    urdf_launch_include = IncludeLaunchDescription(
        ...
        launch_arguments={
            "urdf_package": "yolo_realsense2_models",
